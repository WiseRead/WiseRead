...
export class WiseReadLink {
  ...
  /** @type {string} */
  _source = WiseReadParams.source.default
  ...
  constructor (inLinkOrObject) {
    if (_.isString(inLinkOrObject) || _.isNil(inLinkOrObject)) {
      ...
      this._source = this.getParamValueOrDefault(inLink, WiseReadParams.source)
  ...
  /** @return {number?} */
  get cstart () {
    if (this._configpath || this._source) {
      return this._cstart
    }
 ...
 */
export function splitParamToTypeValue (param) {
  const [type, ...valueArray] = param.split(':')
